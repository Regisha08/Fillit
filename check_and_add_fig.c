...
/*   Updated: 2019/09/19 22:39:03 by aponomar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		pre_check_add(char *area, t_fig *get_fig)
{
	...
	if (get_fig->arr[0] != 0)
		get_fig->j = (get_fig->tmp_j + (((get_fig->arr[get_fig->i]) / 5) * width_area(area)) + ((get_fig->arr[get_fig->i]) % 5)) - get_fig->arr[0];
	else
		get_fig->j = (get_fig->tmp_j + (((get_fig->arr[get_fig->i]) / 5) * width_area(area)) + ((get_fig->arr[get_fig->i]) % 5));
	if (area[get_fig->j] == '.')
	{
		get_fig->ch_c++;
		get_fig->i++;
...
char		*check_add_fig(char *area, t_fig *get_fig)
{
	...
	while (area[get_fig->j] != '\0' && get_fig->ch_c != 4)
	{
		...
		if (area[get_fig->j] == '\0' && get_fig->ch_c != 4)
		{
			...
			get_fig->del = 1;
			printf("\n------------\nNeed more space!!!!\n------------\n");
			printf("I can't include this figure: \n%s\n!!!!", get_fig->fig_save);
			return (area);
		}
